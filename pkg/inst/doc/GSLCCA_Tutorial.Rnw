...
After checking the summary, we can view the fitted model using the plot command:
\begin{center}
@ 
...
\begin{center}  
@ 
<<plot_signatures, fig = TRUE, eps = FALSE>>=
...
\begin{Verbatim}
> write.table(signatures(result), "clonidine signatures.txt", sep = "\t")
\end{Verbatim}
@ 
...
\begin{Verbatim}
> sig1 <- read.delim("Compound1 signatures.txt", check.names = FALSE)
> sig2 <- read.delim("Compound2 signatures.txt", check.names = FALSE)
\end{Verbatim} 
...
The first two arguments of the snapshot function are simply the two sets of 
signatures; setting \Rfunarg{nomalise = TRUE} means that the signatures will be 
...
and makes the signatures comparable across experiments. The \Rfunarg{names}
argument provides names for the two compounds, which are used in the default
titles/legends of the plots produced.

The function can return a number of different plots, as specified by the
\Rfunarg{plot} argument. The first option is \texttt{"signatures"} which
produces a plot of the signatures for each compound, after normalisation. The
\texttt{"mean"} option displays the mean signatures for each compound on the 
same plot. Finally the \texttt{"pvalue"} plot produces a \textit{Manhattan} 
plot of the p-values from the t tests comparing the signatures at each frequency. The p-values are corrected for multiple testing using Benjamini and Hochberg's false discovery rate, by default, alternative options can be specified using the
\Rfunarg{p.adjust.method} argument.

\begin{Verbatim}
> par(mfrow = c(2,2))
> snapshot(sig1, sig2, names = c("Compound1", "Compound2"), 
>         plot = c("signatures", "mean", "pvalue"),
>         normalise = TRUE)
\end{Verbatim}
\begin{figure}[!ht]
    \centering
    \includegraphics{snapshot_multiplot}
\end{figure}
...
par(mfrow = c(1,1))
snapshot(sig1, sig2, names = c("Compound1", "Compound2"), 
         plot = "compact", normalise = TRUE)
\end{Verbatim}
This produces a plot of the mean signatures for each compound, color-coded by
the significance level of the t test at each frequency. As before, this shows significant differences between the mean signatures over the conventional theta and alpha frequency bands.

\begin{figure}[!ht]
    \centering
    \includegraphics{snapshot_compact}
\end{figure}
