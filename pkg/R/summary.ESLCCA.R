summary.ESLCCA <- function(x, digits = max(3, getOption("digits") - 3), ...){
    cat("\nCall:\n", deparse(x$call), "\n", sep = "", fill = TRUE)
    ...
    cat(ifelse(ns != 1, "\nMean signature:\n", "Signature:\n"))
    print.default(format(colMeans(x$ycoef), digits = digits), quote = FALSE)

    cat(ifelse(ns != 1, "\nMean fitted values:\n", "Fitted values:\n"))
    print(format(cast(x$scores, time ~ treatment, value = "xscores", fun.aggregate = mean),
                 digits = digits))
}
